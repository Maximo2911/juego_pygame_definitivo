...
class Player:
    ...
    def do_movement(self, delta_ms, plataform_list):  
        ...
        # print(self.flag_impact)
        if self.flag_impact:
            self.time_impact += delta_ms
            if self.time_impact > 500:
                self.flag_impact = False
                self.time_impact = 0
    ...
    def idle_action(self):
        if not self.is_jump and not self.is_shoot and not self.flag_dead:
    ...
    def shoot_sequence(self, musica):
        if not self.is_jump and not self.flag_dead:
    ...
    def receive_attack(self, enemies_list, bullet_boss, musica):
        for enemy in enemies_list:
            if self.collition_rect.colliderect(enemy.collition_rect):
                if self.flag_impact == False:  #!flag.impact
                        musica.hit.play()
    ...
    def jump_sequence(self, direction, plataform_list):
        # print("lLAMA")
        if self.is_on_plataform(plataform_list) and not self.flag_dead and not self.is_jump:
            # print("ENTRA")
    ...
    def events(self, keys, plataform_list, delta_ms, music):
            ...
            if keys[K_LEFT] and keys[K_SPACE]: # Salto izquierda
                # if(self.time_events - self.time_jump) > COOLDOWN_JUMP:
                    self.jump_sequence("LEFT", plataform_list)
                    # self.time_jump = self.time_events
